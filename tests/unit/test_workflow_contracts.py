...
def workflow_definition():
    return {
        ...
        "transitions": {
            "start": [{"condition": "inspect", "target": "future"}],
            "future": [{"condition": "revise", "target": "start"}],
        },
    }
...
class WorkflowRegistryTests(unittest.TestCase):
    ...
    def test_loads_thick_cyclic_state_space_without_executing_it(self):
        ...
        self.assertEqual("Begin work", definition.states["start"].context)
        self.assertEqual("future", definition.outgoing("start")[0].target)
        self.assertEqual("start", definition.outgoing("future")[0].target)
    ...
    def test_unknown_and_invalid_workflows_fail_explicitly(self):
        ...
        raw = workflow_definition()
        raw["states"]["start"]["role_context"] = "planner"
        with self.assertRaisesRegex(InvalidWorkflowStateError, "固定 Agent"):
            self._loader(raw).load("demo")
    ...
    def test_workflow_initial_state_changes_runtime_visibility(self):
        with tempfile.TemporaryDirectory() as workspace:
            ...
            try:
                ...
                before = harness.build_context(workflow.id)
                self.assertEqual({"echo"}, direct_ids)
                self.assertEqual({"echo"}, {item.id for item in before.capabilities})
                self.assertIsNone(before.workflow)
                self.assertEqual(
                    "general_task",
                ...
                self.assertEqual("active", workflow_context.workflow["current_state"]["id"])
            finally:
                harness.close()
